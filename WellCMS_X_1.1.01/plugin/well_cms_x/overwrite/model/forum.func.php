...
function forum_list_cache()
{
    ...
    if ($forumlist === NULL) {
        ...
        cache_set('forumlist', $forumlist, 3600);
    }
...
function forum_list_access_filter($forumlist, $gid, $allow = 'allowread') {
    ...
    // hook model_website_forum_list_access_filter_before.php
    ...
    // hook model_website_forum_list_access_filter_after.php
    // hook model_forum_list_access_filter_start.php

    foreach($forumlist_filter as $fid=>$forum) {
        // hook model_website_forum_list_access_filter_access_start.php
        if(empty($forum['accesson']) && empty($group[$allow]) || !empty($forum['accesson']) && empty($forum['accesslist'][$gid][$allow])) {
            // hook model_website_forum_list_access_filter_access_before.php
            unset($forumlist_filter[$fid]);
            unset($forumlist_filter[$fid]['modlist']);
            // hook model_website_forum_list_access_filter_access_after.php
        }
        unset($forumlist_filter[$fid]['accesslist']);
        // hook model_website_forum_list_access_filter_access_end.php
    }
    // hook model_website_forum_list_access_filter_end.php
