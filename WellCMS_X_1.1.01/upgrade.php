...
$sql = "ALTER TABLE  `{$db->tablepre}website_thread` CHANGE  `subject`  `subject` VARCHAR( 128 ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT  '';";
$r = db_exec($sql);

$sql = "ALTER TABLE  `{$db->tablepre}website_thread` CHANGE  `brief`  `brief` VARCHAR( 120 ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT  '';";
$r = db_exec($sql);

$sql = "ALTER TABLE  `{$db->tablepre}website_thread` CHANGE  `keyword`  `keyword` VARCHAR( 64 ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT  '';";
$r = db_exec($sql);

$sql = "ALTER TABLE  `{$db->tablepre}website_thread` CHANGE  `description`  `description` VARCHAR( 120 ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT  '';";
$r = db_exec($sql);

$upgrade =  APP_PATH . 'upgrade.php';
file_exists($upgrade) AND unlink($upgrade);
