...
function user_token_get()
{
    ...
    if (!$_uid) {
        //setcookie($conf['cookie_pre'] . 'token', '', $time - 86400, '');
    }
...
function user_token_set($uid)
{
    ...
    setcookie($conf['cookie_pre'] . 'token', $token, $time + 8640000, $conf['cookie_path']);
...
function user_token_clear()
{
    $conf = GLOBALS('conf');
    $time = GLOBALS('time');
    setcookie($conf['cookie_pre'] . 'token', '', $time - 8640000, $conf['cookie_path']);
