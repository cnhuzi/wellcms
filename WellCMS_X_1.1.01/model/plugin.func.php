...
define('PLUGIN_OFFICIAL_URL', DEBUG == 4 ? 'http://plugin.x.com/' : 'http://plugin.wellcms.cn/');
...
function _include($srcfile)
{
    ...
    if (!is_file($tmpfile) || DEBUG > 1) {
        ...
        if (file_ext($tmpfile) == 'php' && DEBUG == 0 && $conf['compress'] > 0) {
            ...
            file_put_contents_try($tmpfile, $s);
        } elseif (file_ext($tmpfile) == 'htm' && DEBUG == 0 && $conf['compress'] > 0) {
            $s = plugin_compile_srcfile($tmpfile);
            $s = well_compress_html($s, $conf['compress']);
            file_put_contents_try($tmpfile, $s);
        } else {
            $s = plugin_compile_srcfile($tmpfile);
            file_put_contents_try($tmpfile, $s);
...
function well_compress_html($string, $compress = 0)
{
    if ($compress == 1) {
        //$string = preg_replace("~>\s+\r\n~", ">", $string); //清除换行符
        // 只压缩html代码，不压缩js，jq中的空行，换行等。
        return preg_replace("~>\s+<~", "><", $string);
    } elseif ($compress == 2)  {
        // 删除双斜杠 // 但是会把html的超链接也干掉
...
function plugin_official_list_cache()
{
    ...
    if ($s === NULL) {
        $url = PLUGIN_OFFICIAL_URL . "plugin-all.htm"; // 获取所有插件
        $s = http_get($url);
...
function plugin_read_by_dir($dir, $local_first = TRUE)
{
    ...
    !isset($local['software_version']) && $local['software_version'] = '1.0';
    ...
    !isset($official['software_version']) && $official['software_version'] = '1.0';
